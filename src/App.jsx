...
function ensureGeographyLayers(map) {
  if (!map.getSource('outside-selected-geography')) {
    map.addSource('outside-selected-geography', {
      type: 'geojson',
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const handleLoad = () => {
      ...
      try {
        ...
        if (pending) {
          renderGeography(map, pending.geography, pending.feature, pending.animate);
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const timerId = window.setTimeout(async () => {
      ...
      try {
        const results = await searchTigerGeographies(query, {
  ...
  const selectGeography = async (location) => {
    ...
    try {
      const feature = await fetchTigerBoundary(location, {
        signal: controller.signal,
      });
      ...
      if (!bounds || !center) {
        throw new Error('No usable map extent returned.');
      }

      const canonical = canonicalOnboardingGeography(
        location,
        feature,
        center,
        bounds,
  ...
  const geographyName = previewGeography
    ? geographyLabel(previewGeography)
    : '';
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#0B0B0D] p-4 font-sans text-[#F7F3EA] antialiased">
      <div
        ...
        aria-label="Interactive 3D RFxchange map"
      />
      ...
      <div
        className={`pointer-events-none absolute inset-0 z-0 transition-opacity duration-1000 ${
          step >= 3 && previewGeography
            ? 'opacity-[0.04]'
            : 'bg-[radial-gradient(circle_at_center,transparent_0%,#0B0B0D_100%)] opacity-55'
        }`}
      />
          ...
          <div className="border-b border-[#F7F3EA]/5 px-6 pb-6 pt-10 text-center sm:px-10 sm:pt-12">
            <div className="mb-6 flex items-center justify-center gap-3">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-[#D6A23A] to-[#a37822] shadow-lg shadow-[#D6A23A]/20">
                <span className="text-xl font-bold tracking-tight text-[#0B0B0D]">
                  RF
                </span>
              </div>
            ...
            <p className="mx-auto max-w-sm text-sm text-[#F7F3EA]/60">
              Create your business account and connect with your local business
              community.
            </p>
          </div>
              ...
              <span className="relative mt-0.5 flex items-center justify-center">
                ...
                <span className="flex h-5 w-5 items-center justify-center rounded-md border-2 border-[#F7F3EA]/30 bg-[#0B0B0D]/50 transition-colors group-hover:border-[#D6A23A]/70 peer-checked:border-[#D6A23A] peer-checked:bg-[#D6A23A]">
                  {formData.termsAccepted && (
                    <ShieldCheck size={14} className="text-[#0B0B0D]" />
                  )}
                </span>
              </span>
      ...
      {step === 2 && (
        <div className="relative z-10 w-full max-w-lg rounded-[32px] border border-[#D6A23A]/30 bg-[#252932]/70 p-8 text-center shadow-[0_0_60px_rgba(0,0,0,0.8)] backdrop-blur-2xl animate-[fadeIn_0.5s_ease-out] sm:p-10">
          ...
          <h2 className="mb-4 text-3xl font-semibold tracking-tight">
            Welcome to The RFxchange.
          </h2>
          <p className="mb-10 text-lg leading-relaxed text-[#F7F3EA]/80">
            Choose the area where your business is based to begin exploring
            your local network.
          </p>
          <button
            type="button"
        ...
        <div className="absolute left-1/2 top-8 z-20 w-[calc(100%-2rem)] max-w-2xl -translate-x-1/2">
          <div className="rounded-[32px] border border-[#F7F3EA]/20 bg-[#252932]/88 p-6 shadow-2xl backdrop-blur-2xl animate-[slideDown_0.5s_ease-out] sm:p-8">
            ...
            <p className="mb-6 text-center text-sm text-[#F7F3EA]/60">
              Search by city, county, ZIP code, or metro area.
            </p>
            ...
            <div className="relative">
              <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-5">
                {isSearching ? (
                  <LoaderCircle
                    size={20}
                    className="animate-spin text-[#D6A23A]"
                  />
                ) : (
              ...
              {(searchResults.length > 0 ||
                ...
                <div className="absolute left-0 right-0 top-full z-30 mt-3 max-h-[22rem] overflow-y-auto rounded-2xl border border-[#F7F3EA]/10 bg-[#252932]/97 shadow-2xl backdrop-blur-xl">
                  {searchResults.map((location) => (
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        <span className="block text-sm text-[#F7F3EA]/50">
                          {location.type}
                        </span>
                      </span>
                    </button>
                  ...
                  {!isSearching &&
                    ...
                    !searchError &&
                    !locationError && (
                      <div className="px-5 py-4 text-sm text-[#F7F3EA]/60">
                        No matches found. Try another place name, county, state,
                        or ZIP code.
                      </div>
                    )}

                  {searchError && (
                    <div className="px-5 py-4 text-sm text-red-200">
                      {searchError}
                    </div>
                  )}
      ...
      {step === 3 && previewGeography && (
        <>
          ...
          >
            <Box
              size={17}
              className={is3D ? 'text-[#D6A23A]' : 'text-[#F7F3EA]/50'}
            />
            {is3D ? '3D' : '2D'}
          </button>
          ...
          <div className="absolute bottom-6 left-1/2 z-20 flex w-[calc(100%-2rem)] max-w-3xl -translate-x-1/2 flex-col items-stretch gap-5 rounded-[32px] border border-[#D6A23A]/50 bg-[#252932]/92 px-6 py-5 shadow-[0_20px_60px_rgba(0,0,0,0.8)] backdrop-blur-2xl animate-[slideUp_0.5s_ease-out_1.1s_both] md:flex-row md:items-center md:gap-6 md:px-8">
            <div className="min-w-0 flex-1">
              <div className="mb-1 text-xs font-semibold uppercase tracking-wider text-[#F7F3EA]/60">
                Your area
              </div>
              <div className="truncate text-2xl font-bold text-[#F7F3EA]">
                {geographyName}
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2 sm:flex-row">
              <button
                type="button"
                onClick={handleChangeGeography}
                className="rounded-xl border border-[#F7F3EA]/20 px-5 py-3 font-semibold text-[#F7F3EA] transition hover:bg-[#F7F3EA]/5"
              >
                Change
              </button>
              <button
                ...
                disabled={isEntering}
                className="flex items-center justify-center gap-2 whitespace-nowrap rounded-xl bg-[#D6A23A] px-6 py-3 font-bold text-[#0B0B0D] transition hover:-translate-y-0.5 hover:bg-[#e4b553] disabled:cursor-wait disabled:opacity-60"
              >
                {isEntering ? (
                  <>
      ...
      {step === 4 && previewGeography && (
        <>
          <div className="absolute left-4 right-4 top-4 z-20 flex items-center justify-between gap-3 md:left-6 md:right-6">
            <div className="flex min-w-0 items-center gap-3 rounded-2xl border border-[#D6A23A]/35 bg-[#17191E]/90 px-4 py-3 backdrop-blur-xl">
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#D6A23A] font-bold text-[#0B0B0D]">
                RF
              </div>
            ...
            <div className="flex shrink-0 gap-2">
              <button
                type="button"
                onClick={handleChangeGeography}
                className="rounded-xl border border-[#F7F3EA]/15 bg-[#17191E]/90 px-4 py-3 text-sm font-semibold backdrop-blur-xl transition hover:border-[#D6A23A]/50"
              >
                Change area
              </button>
              <button
                type="button"
                onClick={toggle3D}
                className="flex items-center justify-center gap-2 rounded-xl border border-[#F7F3EA]/15 bg-[#17191E]/90 px-4 py-3 text-sm font-semibold backdrop-blur-xl"
                aria-pressed={is3D}
              >
                <Box
                  size={17}
      ...
      <style>{`
        ...
        .maplibregl-ctrl-attrib {
          background: rgb(11 11 13 / 0.78) !important;
